...
export default class ChatServices {
  ...
  getChats = async (userId: string) => {
    try {
        ...
        const unreadCounts = await this.chatRepository.findUnreadCountByUserId(userId)
    
        const unreadCountMap = new Map(unreadCounts.map(item => [item.chatRoomId, item.unreadCount]));
  ...
  createChatRoom = async (userId1: string, userId2: string) => {
    try {
      ...
      return result;
        ...
        console.log(error instanceof Error?error.message:"error occured from createChatRom")
